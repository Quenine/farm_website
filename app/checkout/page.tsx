import { CheckoutForm } from "@/src/components/checkout/checkout-form";
import { PageShell } from "@/src/components/ui";
import { getCheckoutDeliveryData } from "@/src/lib/delivery-data";

export const dynamic = "force-dynamic";

export default async function CheckoutPage() {
  const { zones, settings } = await getCheckoutDeliveryData();

  return (
    <PageShell>
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-bold text-green-950">Checkout</h1>
        <p className="mt-3 text-stone-700">
          Create your order now. Payment will be connected in the next step.
        </p>
        <div className="mt-8">
          <CheckoutForm zones={zones} settings={settings} />
        </div>
      </section>
    </PageShell>
  );
}
