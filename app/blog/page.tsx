  ...
  return (
    <PageShell>
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {data.posts.length ? <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{data.posts.map((post) => <PostCard key={post.id} post={post} />)}</div> : <div className="mt-8 rounded-lg bg-white p-8 text-center shadow-sm"><h2 className="text-2xl font-bold text-green-950">Agribusiness guides are being prepared</h2><p className="mt-2 text-stone-600">Agribusiness guides, practical farming resources and product recommendations are being prepared. Check back soon.</p><div className="mt-5 flex flex-wrap justify-center gap-3"><Link href="/tools" className="rounded-full border border-green-800 px-4 py-2 text-sm font-bold text-green-950">Open Tools</Link><Link href="/resources" className="rounded-full border border-green-800 px-4 py-2 text-sm font-bold text-green-950">View Resources</Link><Link href="/shop" className="rounded-full bg-green-800 px-4 py-2 text-sm font-bold text-white">Shop Products</Link></div></div>}
