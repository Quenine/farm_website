...
export async function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-stone-100"><div className="sticky top-0 z-40 flex items-center justify-between bg-green-950 px-4 py-3 text-white lg:hidden"><details className="group"><summary className="cursor-pointer list-none rounded-lg border border-white/30 px-4 py-2 font-bold">Menu</summary><div className="fixed inset-0 top-[68px] overflow-y-auto bg-green-950 p-4">{sidebar}</div></details><AdminNotificationBell initial={notifications}/></div><div className="mx-auto grid max-w-7xl gap-6 px-3 py-4 sm:px-4 lg:grid-cols-[260px_minmax(0,1fr)] lg:py-6"><aside className="hidden rounded-lg bg-green-950 p-4 text-white shadow-sm lg:block lg:min-h-[calc(100vh-48px)]">{sidebar}</aside><main className="min-w-0"><div className="mb-3 hidden justify-end lg:flex"><AdminNotificationBell initial={notifications}/></div>{children}</main></div></div>;
}
