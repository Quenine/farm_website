...
export function ManualCandidateForm({ campaigns }: { campaigns: SalesScoutCampaignDto[] }) {
  ...
  const updateChannel=(index:number,patch:Partial<Channel>)=>setChannels((current)=>current.map((row,i)=>{
  ...
  const choices=(preview?.allowedResolutionChoices as Array<{choice:string;prospectId?:string}>|undefined)??[];
  ...
  return <div className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]"><form onSubmit={onPreview} className="grid gap-5 rounded-xl border bg-white p-5">
    <fieldset className="grid gap-3 sm:grid-cols-2"><legend className="mb-3 text-xl font-bold">Campaign and business</legend>
      <label className="font-bold">Active campaign<select value={campaignId} onChange={e=>setCampaignId(e.target.value)} className={`${input} mt-1`}>{campaigns.map(c=><option key={c.campaignId} value={c.campaignId}>{c.name}</option>)}</select></label>
      ...
      <label className="font-bold">City<input name="city" required defaultValue={campaign?.city} className={`${input} mt-1`}/></label>
      <label className="font-bold">State<input name="state" defaultValue={campaign?.state??""} className={`${input} mt-1`}/></label>
      <label className="font-bold">Country<input name="country" required defaultValue={campaign?.country} className={`${input} mt-1`}/></label>
      ...
      <label className="font-bold">Observed at<input name="observedAt" type="datetime-local" required defaultValue={new Date().toISOString().slice(0,16)} className={`${input} mt-1`}/></label>
    ...
    <fieldset className="space-y-3"><legend className="text-xl font-bold">Public contact channels</legend>{channels.map((row,index)=><div key={index} className="grid gap-2 rounded-lg bg-stone-50 p-3 sm:grid-cols-2">
      ...
      <div className="flex items-center justify-between"><label className="flex gap-2"><input type="radio" name="primaryChannel" checked={row.primary} onChange={()=>updateChannel(index,{primary:true})}/>Primary</label>{channels.length>1?<button type="button" onClick={()=>setChannels(c=>c.filter((_,i)=>i!==index))} className="font-bold text-red-700">Remove</button>:null}</div>
    </div>)}{channels.length<5?<button type="button" onClick={()=>setChannels(c=>[...c,blank()])} className="rounded-full border px-4 py-2 font-bold">Add channel</button>:null}</fieldset>
  ...
  <aside className="rounded-xl border bg-white p-5"><h2 className="text-xl font-bold">Preview before capture</h2>{preview?<div className="mt-4 space-y-4"><p><strong>Normalized business:</strong> {String((preview.normalizedCandidate as Record<string,unknown>)?.normalizedBusinessName??"—")}</p><p><strong>Score:</strong> {String((preview.scorePreview as Record<string,unknown>)?.score??"—")} · {(preview.scorePreview as Record<string,unknown>)?.qualified?"Eligible":"Not eligible"}</p><p><strong>Exact match:</strong> {preview.exactMatch?"Existing identity found":"None"}</p><p><strong>Soft warnings:</strong> {Array.isArray(preview.softMatchWarnings)?preview.softMatchWarnings.length:0}</p><div className="space-y-2">{choices.map((choice,index)=><button key={index} type="button" disabled={pending} onClick={()=>capture(choice)} className="block w-full rounded-full bg-green-800 px-4 py-2 font-bold text-white">{choice.choice==="create_new"?"Create new prospect":`Attach to ${choice.prospectId}`}</button>)}</div><p className="text-xs text-stone-500">Attachment is always an explicit owner choice. No prospect is silently merged.</p></div>:<p className="mt-3 text-stone-600">Complete the normal fields and preview. Capture is unavailable until preview succeeds.</p>}</aside></div>;
}
