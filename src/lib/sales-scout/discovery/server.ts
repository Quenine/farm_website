...
export async function runSalesScoutDiscovery(input:{campaignId:string}):Promise<DiscoveryRunSummary>{
  ...
  try{
    ...
    const payload={providerTaskId,rawResultCount:research.structuredSeedCount,structuredSeedCount:research.structuredSeedCount,discardedSourceDocumentCount:research.discardedSourceDocumentCount,enrichmentAttemptedCount:research.enrichmentAttemptedCount,enrichmentCompletedCount:research.enrichmentCompletedCount,officialWebsitesResearched:research.officialWebsitesResearched,providerCredits:research.providerCredits,warnings:research.warnings,resolvedTerritory:{latitude:research.resolvedTerritory.latitude??null,longitude:research.resolvedTerritory.longitude??null},candidates};
    ...
    const dto=completed.data as Record<string,unknown>; return{runId,providerTaskId,rawResultCount:research.structuredSeedCount,stagedCandidateCount:Number(dto.stagedCandidateCount),exactDuplicateCount:Number(dto.exactDuplicateCount),providerCredits:research.providerCredits,mappingIssueCount,manualReviewReadyCount:research.manualReviewReadyCount,outreachReadyCount:research.outreachReadyCount};
