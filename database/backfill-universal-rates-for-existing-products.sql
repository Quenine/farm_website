-- Emergency backfill for universal product delivery rates for Noble Farms / Shields Farms.
-- Run anytime after database/normalize-legacy-products.sql on each farm Supabase project.
-- This script does not delete orders, products, product media, or exact-city delivery-rate overrides.
-- It only upserts state-wide city = 'All' fallback rows for active fixed-price orderable products with price > 0.
-- Checkout prefers exact state/city rates first, then state/city = All fallback rates.
...
--   pickup_point: package_size = 1, first_package_fee = 10000, extra_package_fee = 3000
--   home_delivery: package_size = 1, first_package_fee = 15000, extra_package_fee = 3000
--   farm_pickup: package_size = 1, first_package_fee = 0, extra_package_fee = 0
-- Admin can add exact city rows such as Lagos/Lekki or Oyo/Iseyin without this seed overwriting them.