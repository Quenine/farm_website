...
begin
  ...
  -- First pass: validate every record before any candidate or membership mutation.
  for v_item in select value from jsonb_array_elements(p_payload->'candidates') loop
    if jsonb_typeof(v_item)<>'object' or nullif(trim(v_item->>'providerSourceId'),'') is null or length(trim(v_item->>'providerSourceId'))>300 or nullif(trim(v_item->>'businessName'),'') is null or length(trim(v_item->>'businessName'))>200 or nullif(trim(v_item->>'normalizedBusinessName'),'') is null or length(trim(v_item->>'normalizedBusinessName'))>200 or nullif(v_item->>'observedAt','') is null or jsonb_typeof(v_item->'providerCategoryIds') is distinct from 'array' or jsonb_typeof(v_item->'additionalCategories') is distinct from 'array' or jsonb_typeof(v_item->'mappingIssues') is distinct from 'array' or jsonb_typeof(v_item->'scoreFactors') is distinct from 'array' then raise exception using errcode='22023',message='discovery candidate payload is invalid'; end if;
    begin perform (v_item->>'observedAt')::timestamptz; exception when others then raise exception using errcode='22023',message='discovery candidate payload is invalid'; end;
    if exists(select 1 from jsonb_array_elements(v_item->'providerCategoryIds') x where jsonb_typeof(x)<>'string') or exists(select 1 from jsonb_array_elements(v_item->'additionalCategories') x where jsonb_typeof(x)<>'string') or exists(select 1 from jsonb_array_elements(v_item->'mappingIssues') x where jsonb_typeof(x)<>'string') then raise exception using errcode='22023',message='discovery candidate payload is invalid'; end if;
    if nullif(v_item->>'mappedCampaignCategory','') is not null and not exists(select 1 from unnest(v_campaign.target_categories) x where lower(trim(x))=lower(trim(v_item->>'mappedCampaignCategory'))) then raise exception using errcode='22023',message='discovery candidate payload is invalid'; end if;
    if v_item ? 'preparedScore' and v_item->>'preparedScore' is not null and ((v_item->>'preparedScore')::numeric<>trunc((v_item->>'preparedScore')::numeric) or (v_item->>'preparedScore')::integer not between 0 and 100 or v_item->>'scoreVersion'<>'ng-city-b2b-v1') then raise exception using errcode='22023',message='discovery candidate payload is invalid'; end if;
    if coalesce((v_item->>'softMatchWarningCount')::integer,0)<0 or (v_item->>'ratingCount' is not null and (v_item->>'ratingCount')::integer<0) or (v_item->>'ratingValue' is not null and (v_item->>'ratingValue')::numeric<0) or (v_item->>'latitude' is not null and (v_item->>'latitude')::numeric not between -90 and 90) or (v_item->>'longitude' is not null and (v_item->>'longitude')::numeric not between -180 and 180) or (v_item->>'countryCode' is not null and length(trim(v_item->>'countryCode'))<>2) then raise exception using errcode='22023',message='discovery candidate payload is invalid'; end if;
    if nullif(v_item->>'exactMatchingProspectId','') is not null then begin v_exact_id:=(v_item->>'exactMatchingProspectId')::uuid; if not exists(select 1 from public.marketing_prospects where id=v_exact_id) then raise exception using errcode='22023',message='discovery candidate payload is invalid'; end if; exception when invalid_text_representation then raise exception using errcode='22023',message='discovery candidate payload is invalid'; end; end if;
  end loop;
